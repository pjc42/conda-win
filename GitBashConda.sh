...
#   activate envname
#   activate-list, same as conda envs list
#   deactivate 
...
function activate() {
    ...
    local target_envname="$1"

    # assumes that path_anaconda3 is set, this is set in the .bashrc or something like that
    local anacondaInstallPath=$path_anaconda_3

    # build anaconda env path
    local anaconda_envs="${anacondaInstallPath}/envs"

    # build abs path to new env
    local anaconda_target_env_path="${anaconda_envs}/${target_envname}"
...
function activate-list() {
    ...
    if [[ ! -z "$CONDA_CURRENT_ENV" ]]; then
        # ! empty string
        # CONDA_CURRENT_ENV contains a value so echo current env
        echo
        echo "CONDA_CURRENT_ENV= ${CONDA_CURRENT_ENV}"
        echo
    fi

    local env_no=1  # counter for envs
    local item      
    local envname
    for item in ${anaconda_envs}/* -maxdepth 1; do
        if [[ -d "${item}" ]] ; then
            # if item is a folder then probably a conda env
            envname=$(basename $item)
            echo -e "[${env_no}] \t ${envname}"
            env_no=$((env_no + 1))
        fi
    done
